...
export default function Checkout() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // 2. Create order
      const orderNumber = `PAP-${Date.now()}`;
      const { data: order, error: orderErr } = await supabase
        ...
        .insert({
          ...
          shipping_address: { ...shippingData, country: "India" },
      ...
      clearCart();
      // Redirect to order confirmation (payment gateway integration requires server-side secrets)
      setLocation(`/payment/status?txnId=${order.order_number}&status=pending`);
    } catch (err) {
      console.error(err);
      toast({
        title: "Order Error",
                ...
                <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <form onSubmit={shippingForm.handleSubmit(onShippingSubmit)} className="space-y-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select onValueChange={(val) => shippingForm.setValue("state", val)} defaultValue={shippingForm.getValues("state")}>
